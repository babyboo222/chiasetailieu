...
public class UserService {
    ...
    public User regisUser(User user) {
        ...
        userRepository.findByEmail(user.getEmail())
                .ifPresent(exist->{
                    if(exist.getActivation_key() != null){
                        throw new MessageException("Tài khoản chưa được kích hoạt, hãy truy cập vào email :"+user.getEmail()+" để kích hoạt tài khoản", 330);
                    }
...
//                "Để kích hoạt tài khoản của bạn, hãy nhập mã xác thực phía dưới để xác nhận tài khoản<br><br>" +
//                "<a style=\"background-color: #2f5fad; padding: 10px; color: #fff; font-size: 18px; font-weight: bold;\">"+user.getActivation_key()+"</a>",false, true);
    ...
    public Boolean checkUser(Optional<User> users){
        if(users.isPresent() == false){
            ...
            throw new MessageException("Tài khoản chưa được kích hoạt -"+users.get().getEmail(), 300);
        }
