...
public interface UserRepository extends JpaRepository<User,Long> {
    ...
    Optional<User> findByEmailAndUserId(String email, Long userId);
    ...
    @Query(value = "SELECT * from users u where u.authority_name = ?1 and Month(u.created_date) = MONTH(CURRENT_DATE()) and YEAR(u.created_date) = YEAR(CURRENT_DATE)", nativeQuery = true)
    public List<User> getNewUser(String role);
